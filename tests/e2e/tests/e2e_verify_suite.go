...
func (r *EvidenceE2ETestsRunner) RunVerifyEvidenceSuite(t *testing.T) {
	...
	t.Run("WithAttachment", func(t *testing.T) {
		r.RunVerifyEvidenceWithAttachment(t)
	})
}
...
// RunVerifyEvidenceWithAttachment ensures verify works for evidence with attachments using the standard CLI harness.
func (r *EvidenceE2ETestsRunner) RunVerifyEvidenceWithAttachment(t *testing.T) {
	t.Log("=== Verify Evidence - With Attachment ===")
	...
	evidenceVerifications, ok := jsonData["evidenceVerifications"].([]any)
	require.True(t, ok, "verify output should include evidenceVerifications")
	require.NotEmpty(t, evidenceVerifications, "verify output should include at least one evidence verification")

	firstVerification, ok := evidenceVerifications[0].(map[string]any)
	require.True(t, ok, "first evidence verification must be an object")

	verificationResult, ok := firstVerification["verificationResult"].(map[string]any)
	require.True(t, ok, "verificationResult should be present")
	require.Equal(t, "success", verificationResult["attachmentsVerificationStatus"])
	t.Log("=== ✅ Verify Evidence with Attachment completed ===")
}
